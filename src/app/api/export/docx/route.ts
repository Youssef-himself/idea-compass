  ...
  try {
    ...
    return new NextResponse(buffer as unknown as BodyInit, {
      ...
      headers: {
        ...
        'Content-Disposition': `attachment; filename="IdeaCompass-${isLiteReport ? 'Lite-' : ''}Report-${report.id}.docx"`,
      },
    });
...
async function generateLiteReportDOCX(report: GeneratedReport): Promise<Document> {
  const children = [];

  // Title with Lite badge
  children.push(
    new Paragraph({
      children: [
        new TextRun({
  ...
  children.push(
    new Paragraph({
      text: "Basic Market Research Summary",
      alignment: AlignmentType.CENTER,
      spacing: { after: 600 },
    })
  );
  ...
  // Business Opportunities Summary (if available)
  if (report.businessPlans && report.businessPlans.length > 0) {
    children.push(
      new Paragraph({
        text: "Business Opportunities (Summary)",
        heading: HeadingLevel.HEADING_1,
        spacing: { before: 400, after: 400 },
      })
    );
    ...
    report.businessPlans.forEach((plan, planIndex) => {
      ...
      // Market potential and feasibility scores only
      children.push(
        new Table({
          width: {
            size: 100,
  ...
  children.push(
    new Paragraph({
      children: [
        new TextRun({
          text: "🚀 Upgrade to Pro or Premium for the Full Report",
          bold: true,
          size: 24,
          color: 'FF8C00',
        }),
  ...
  children.push(
    new Paragraph({
      text: "This lite version includes only the executive summary, key findings, and basic business opportunity scores.",
      alignment: AlignmentType.CENTER,
      spacing: { after: 200 },
    })
  );
  ...
  const upgradeFeatures = [
    "Detailed business plans with market analysis",
    "Monetization strategies and action plans",
    "Comprehensive market research sections",
    "Strategic recommendations",
    "Full supporting evidence and citations"
  ];
  ...
  children.push(
    new Paragraph({
      children: [
        new TextRun({
          text: "Visit our pricing page to learn more about upgrading!",
          bold: true,
        }),
      ],
      alignment: AlignmentType.CENTER,
  ...
  children.push(
    new Paragraph({
      text: `Generated by IdeaCompass on ${new Date().toLocaleDateString()} • Lite Version`,
      alignment: AlignmentType.CENTER,
    })
  );
  ...
  return new Document({
    ...
    title: `${report.title} - Lite Version`,
    description: "Lite Market Research Report generated by IdeaCompass",
    creator: "IdeaCompass",
    keywords: "market research, reddit analysis, business intelligence, lite version",
  });
}