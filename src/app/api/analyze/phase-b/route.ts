...
import { APIResponse, RedditPost, BusinessIdea, BusinessPlan } from '@/types';
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('Error in phase-b analysis API:', error);
...
async function generateMockBusinessPlans(posts: RedditPost[], selectedIdeas: BusinessIdea[]): Promise<BusinessPlan[]> {
  ...
  for (const idea of selectedIdeas) {
    ...
    if (idea.id.includes('workflow')) {
      plan = {
        id: idea.id,
        title: idea.title,
        coreProblem: 'Small teams struggle with manual, repetitive tasks that consume valuable time and resources, as evidenced by frequent complaints about inefficient workflows and time-consuming processes in the analyzed community discussions.',
        ...
        actionPlan: 'Create a landing page showcasing workflow automation benefits and collect emails from 50 potential users from relevant business communities to validate demand before building the MVP.'
      };
    } else if (idea.id.includes('data')) {
      plan = {
        id: idea.id,
        title: idea.title,
        coreProblem: 'Businesses are drowning in unorganized data across multiple platforms and tools, leading to inefficiency, duplicated efforts, and missed insights as highlighted in numerous data management frustrations in the community posts.',
        ...
        actionPlan: 'Build a prototype that organizes sample business documents and demonstrate it to 20 potential customers from business-focused communities to gather feedback and validation.'
      };
    } else if (idea.id.includes('communication')) {
      plan = {
        id: idea.id,
        title: idea.title,
        coreProblem: 'Remote and distributed teams lose context and efficiency due to fragmented communication across multiple tools, as evidenced by posts about miscommunication and project delays in the analyzed discussions.',
        ...
        actionPlan: 'Create a mockup of the context-aware interface and conduct user interviews with 15 remote team leads from relevant communities to validate the core value proposition.'
      };
    } else if (idea.id.includes('analytics')) {
      plan = {
        id: idea.id,
        title: idea.title,
        coreProblem: 'Business teams struggle to get unified insights from their fragmented data sources, spending hours manually gathering reports instead of making data-driven decisions, as reflected in the community discussions about reporting challenges.',
        proposedSolution: 'A real-time business intelligence platform that automatically aggregates data from multiple sources and provides customizable dashboards with automated insights and alerts.',
        targetAudience: 'Mid-market companies (50-500 employees), data-driven startups, and department managers who need regular business insights but lack dedicated data analysts.',
        keyFeatures: [
          'One-click integrations with 50+ business tools',
          'Drag-and-drop dashboard builder',
          'AI-powered insight generation and anomaly detection',
          'Automated report scheduling and distribution',
          'Mobile-responsive dashboards with offline access'
        ],
        marketPotential: 8,
        feasibility: 6,
        monetization: 'Freemium model: Free (2 data sources), Professional ($49/month for 10 sources), Enterprise ($199/month unlimited sources plus white-labeling).',
        actionPlan: 'Build connections to 5 popular business tools (Google Analytics, Stripe, Salesforce, HubSpot, Slack) and create sample dashboards to demonstrate value to 25 potential users.'
      };
    } else if (idea.id.includes('integration')) {
      plan = {
        id: idea.id,
        title: idea.title,
        coreProblem: 'Non-technical business users face significant barriers when trying to integrate their various software tools, often requiring expensive developer resources or complex IT projects, as shown by integration frustrations in the community data.',
        proposedSolution: 'A visual API integration platform that allows business users to connect applications through simple drag-and-drop interfaces, with pre-built connectors and data transformation tools.',
        targetAudience: 'Small to medium businesses, operations managers, and business analysts who need to connect software tools but lack technical API knowledge.',
        keyFeatures: [
          'Visual workflow designer for API integrations',
          ...
          'Template marketplace for common integration patterns'
        ],
        marketPotential: 8,
        feasibility: 7,
        monetization: 'Subscription tiers: Starter ($29/month for 5 integrations), Professional ($99/month for 25 integrations), Enterprise ($299/month unlimited plus priority support).',
        actionPlan: 'Focus on 10 most requested integrations, build a working demo, and get 30 businesses to trial the platform with their actual integration needs.'
      };
    } else {
  ...
  console.log(`Mock Phase B: Generated ${businessPlans.length} detailed business plans`);
